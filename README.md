...
保留策略按 `0.<minor>` 系列管理：保留两个 `0.x` 系列；最新 `0.x` 系列保留两个小版本；其他保留系列如果没有在更新的 `0.x` 系列发布后继续补发，只保留该系列最新版本；如果旧系列在更新的 `0.x` 系列发布后补发更新，则把补发版本追加进清单并保留补发后的两个版本。
...
我们的 CI 把默认 wrapper、Hermes Agent sandbox 和 Ironclaw release 产物 push 到 `https://hugefiver.cachix.org`，命中即秒拉，避免本机长时间编译 heavy package。
