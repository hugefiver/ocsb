    ...
    in
    {
      ...
      packages = forAllSystems (system:
        let
          ...
          sidecarGate = pkgs.callPackage ./pkgs/sidecar-gate.nix { };
          ...
          mkSandboxBin = { slug, persistSlug ? slug, ironclawSandboxBase }: pkgs.callPackage ./scripts/ironclaw-wrapper.nix {
            inherit slug persistSlug ironclawSandboxBase sidecarGate;
            sidecarTestHookMode = "none";
          };
          ...
          baselineArch = builtins.head (ironclawArchsFor system);
      ...
      checks = forAllSystems (system:
        ...
        in
        {
          ...
          dual-layer-home-test = mkSandbox ({ pkgs, ... }: {
      ...
      devShells = forAllSystems (system:
        ...
        in
        {
          default = pkgs.mkShell {
            packages = defaultTestTools;
          };

          # CI-only native backend environment. Podman comes from the locked
          # root nixpkgs input and is not a product runtime dependency.
          backend-test = pkgs.mkShell {
            packages = defaultTestTools ++ [ pkgs.podman ];
          };
        }
      );
    };
