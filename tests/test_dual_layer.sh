...
HTTP_EXIT=$?
set -e
assert "outer network command exits zero" [ "$HTTP_EXIT" -eq 0 ]
assert "outer network returned an HTTP code" [ -n "$HTTP_CODE" ]
assert "outer sandbox has internet access (got $HTTP_CODE)" [ "$HTTP_CODE" != "000" ]
echo ""
...
# Capture its expected non-zero result rather than swallowing it.
set +e
INNER_NET=$("$SHELL" -c 'curl -s -o /dev/null -w "%{http_code}" --connect-timeout 3 https://httpbin.org/get 2>/dev/null' 2>/dev/null)
INNER_NET_EXIT=$?
set -e
assert "inner network command exits non-zero" [ "$INNER_NET_EXIT" -ne 0 ]
...
"$SHELL" -c "echo WRITTEN_FROM_INNER > $_TEST_FILE" 2>/dev/null
...
SHELL_VERSION=$("$SHELL" --version 2>/dev/null)
...
H_FLAG_LAYER=$("$SHELL" -h -c 'echo $OCSB_DUAL_LAYER' 2>/dev/null)
...
echo 'curl -s -o /dev/null -w "%{http_code}" --connect-timeout 2 https://httpbin.org/get 2>/dev/null' > "$_NET_SCRIPT"
set +e
SCRIPT_NET=$("$SHELL" "$_NET_SCRIPT" 2>/dev/null)
SCRIPT_NET_EXIT=$?
set -e
rm -f "$_NET_SCRIPT"
assert "script invocation network command exits non-zero" [ "$SCRIPT_NET_EXIT" -ne 0 ]
