...
- Wrapper binary is `ocsb-hermes`; flake package alias is `hermes-agent-sandbox`.
...
- Wrapper-reserved env names (`OCSB_HERMES_AGENT_PERSIST_DIR`, `OCSB_HERMES_AGENT_API_KEYS_ENV_FILE`, `OCSB_HERMES_NO_GATEWAY`, `HERMES_HOME`, `MESSAGING_CWD`) must be rejected for `--env ...` with error text mentioning “reserved for the Hermes Agent wrapper”.
- Hermes template runs as non-root, sets `HERMES_HOME=/home/sandbox/.hermes`, `MESSAGING_CWD=/home/sandbox`, initializes minimal config only when absent, and has no DB/Postgres/sidecar behavior.
...
bash tests/test_hermes_agent.sh ./result/bin/ocsb-hermes
