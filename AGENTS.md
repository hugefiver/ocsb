...
- Wrapper-reserved env names (`OCSB_HERMES_AGENT_PERSIST_DIR`, `OCSB_HERMES_AGENT_API_KEYS_ENV_FILE`, `OCSB_HERMES_NO_GATEWAY`, `HERMES_HOME`, `TERMINAL_CWD`) must be rejected for `--env ...` with error text mentioning “reserved for the Hermes Agent wrapper”.
- Hermes template runs as non-root, sets `HERMES_HOME=/home/sandbox/.hermes`, `TERMINAL_CWD=/home/sandbox`, initializes minimal config only when absent, and has no DB/Postgres/sidecar behavior.
